...
| Label quality | *Added 30 Aug 2026 (D33).* **The labels do not reproduce — A13 fails.** On a recheck of 100 pairs (machine) / 50 (human), two judges blind to A1's answer both land near chance against it: kappa(A1, human) = **0.034** (n=50), kappa(A1, `llm:claude-sonnet-5`) = **0.125** (n=100) — 0.031 and 0.113 under binary collapse, and exactly 0.000 over the original 50-pair draw — while the two blind judges agree with each other at kappa = **0.275** (0.425 collapsed, n=50). The failure is in A1's positives: the judge agrees 0.90 on A1's `No Fit` (n=40) but **0.067 on its `Good Fit`** (n=30), reading 26 of those 30 as `No Fit`. **D26's 0.7806 / 0.7188 ceilings are computed off these labels and are therefore soft**; they are not restated here. *Superseded 5 Sep 2026 (D34)* — the human leg closed at 100 pairs and the judge's guide was rewritten, so the n=50 figures above are read beside: kappa(A1, human) = **0.081** and kappa(A1, `llm:claude-haiku-4-5`) = **0.085**, both n=100, against kappa(human, llm) = **0.352** (**0.614** collapsed). **A13 still fails** and the ceilings are still soft; what changed is the judge, not A1. See the [card](cards/A1-resume-job-description-fit.md) defect 5, `plan/2026-08-30-llm-recheck/01-findings.md` and `plan/2026-09-05-prompt-calibration/02-calibration-run.md`. *Amended 6 Sep 2026 (D35)* — no figure here moves; the judge's labels now live in two files, `llm-recheck.csv` (the 100-pair recheck these figures are computed over, 240 rows across 4 runs) and `llm-recheck-full-a1.csv` (option D, the same judge over all 659 A1 pairs, 639 rows, **read by nothing**). Option D carries the pre-D34 instrument `47fc2e48`, so no figure is quoted off it (**Q34**, deferred). The one number that had drifted is the judge's test-retest reliability, widened from n=20 to n=100 in the committed report when option D was dispatched onto D33's run 2; it is back at **0.857** [0.417, 1.000], n=20. See `plan/2026-09-06-recheck-file-split/` |
